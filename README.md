...
Invitations and password resets use Cloudflare Email Service through the restricted `EMAIL` binding, from `noreply@ideagap.org`. A verified sending domain and Email Service permissions are required before phase 8 passes. Local delivery is simulated: Wrangler prints paths to local HTML/text email files under ignored `apps/web/.wrangler/tmp/email`; no real message is sent. Open the local HTML email to follow its password setup link. Never commit these files or paste their links into issue trackers.

Wrangler's current authorization lacks `email_sending:write` (API 2036); the attempted OAuth refresh timed out. Refresh with `bunx --no-install wrangler login --scopes account:read user:read workers:write workers_scripts:write workers_tail:read d1:write zone:read email_sending:write`, complete the browser authorization, then run `bunx --no-install wrangler email sending list`. Onboard `ideagap.org` to Email Sending and verify its DNS before a live delivery check. Receiving mail/Email Routing is not required for this application.
...
The first local administrator is **Ideahub-Admin**, **admin@ideagap.org**. Its random bootstrap password was discarded after login verification. Use `/forgot-password` while the local app is running, enter that email, and open the generated local HTML email from Wrangler's output to set a password. Reset links expire after one hour. No remote administrator was created.
...
## User management and auth auditing

Admins use `/admin/users` to invite users, assign roles, ban/unban, and force password reset. Invitation links expire after 24 hours. Forced reset immediately replaces the old password with a discarded random credential, revokes sessions, invalidates previous reset tokens, and sends a one-hour setup link. Email failures remain visible in the users table; retry with Force reset. Self-demotion and self-ban are blocked server-side.

Better Auth owns account mutations. Its D1 adapter does not combine plugin operations with application audits in one D1 batch, so management writes record an intent before the plugin call and a completion afterward. Investigate unmatched requested/completed audit events before retrying. Last-sign-in timestamps persist beyond session deletion. Email delivery status updates and their audits are atomic; reset tokens/passwords never enter activity metadata.

Phase 8 is verified locally; live email verification is pending. Per the phase gates, vault implementation and final hardening/deployment remain unstarted until that prerequisite passes. See [BUILD_STATUS.md](BUILD_STATUS.md) for exact evidence.
